...
test('CLI remembers install location and works from an unrelated directory', async t => {
  ...
  const env = { ...process.env, HOME: dir, XDG_CONFIG_HOME: join(dir, 'config'), CODEX_HOME: join(dir, 'codex'), CLAUDE_CONFIG_DIR: join(dir, 'claude'), NO_COLOR: '1' };
  delete env.BENBOT_HOME;
  execFileSync(process.execPath, [cli, 'install', '--root', store.root], { cwd: tmpdir(), env });
  assert.equal(await fs.readlink(join(dir, '.agents/AGENTS.md')), join(store.root, 'SOUL.md'));
...
test('soul installs one shared file under both global names and preserves edits', async t => {
  ...
  const { dir, store } = await fixture(t);
  const targets = [join(dir, '.agents/AGENTS.md'), join(dir, '.codex/AGENTS.md'), join(dir, '.claude/CLAUDE.md')];
